    ...
    var r = _avatar[0] % W,
        ...
        a = function (t, e, n, r) {
            var s = r * _rank,
                ...
                u = s * n,
                h = i * n;
            t.css("background-size", u + "px " + h + "px"),
                t.css("background-position", a + "px " + c + "px")
        };
    a(_playerDiv.find(".color"), r, 10, 48),
        a(_playerDiv.find(".eyes"), s, 10, 48),
        a(_playerDiv.find(".mouth"), i, 10, 48),
        _avatar[3] >= 0 ? (_playerDiv.find(".special").show(),
            a(_playerDiv.find(".special"), _avatar[3], 10, 80)) : _playerDiv.find(".special").hide(),
        _owner ? _playerDiv.find(".owner").show() : _playerDiv.find(".owner").hide()
}
...
function saveAvatarData() {
    localStorage.setItem("avatar", JSON.stringify(loginAvatarData))
}
...
function u(t, e) {
    t < 0 || t >= 3 || (e < 0 && (e = z[t] - 1),
        e >= z[t] && (e = 0),
        loginAvatarData[t] = e,
        initFunc(jQuery("#loginAvatar"), loginAvatarData, !1, 2),
        saveAvatarData())
}

function h() {
    jQuery("#modalIdle").modal()
}

function l() {
    if (socket)
        if (game && game.drawingID == game.myID) {
            if (ct.drawCommands.length > 0) {
                if (ct.drawCommands.length > 8) {
                    var t = ct.drawCommands.splice(7);
                    socket.emit("drawCommands", ct.drawCommands),
                        ct.drawCommands = t
                } else
                    socket.emit("drawCommands", ct.drawCommands),
                    ct.drawCommands = []
            ...
            ct.drawCommands.length > 0 && (ct.drawCommands = [])
}

function p() {
    setTimeout(function () {
        jQuery("#rateDrawing").show()
    }, 5e3)
}

function f() {
    jQuery("#rateDrawing").hide()
}

function d() {
    jQuery("#votekickCurrentplayer").prop("disabled", !1)
}

function y() {
    jQuery("#votekickCurrentplayer").prop("disabled", !0)
}

function m() {
    socket && socket.emit("votekickCurrent"),
        y()
}

function g() {
    at.goto("load");
    var t = null === localStorage.getItem("firstLogin");
    t && localStorage.setItem("firstLogin", "");
    var e = !1;
    if (!t) {
...
function v() {
    "undefined" != typeof aipPlayer && (aiptag = aiptag || {},
        aiptag.consented = !0,
...
function k() {
    ...
    r.length > 1 && (o = r[1]);
    var s = jQuery("#loginLanguage").val();
    return {
        name: e[0],
        code: e[1],
        ...
        socket.on("connect", function () {
            socket.on("disconnect", function () {
                    socket.close(),
                        ...
                        game = null,
                        at.goto("login"),
                        jQuery("#modalDisconnect").modal()
                }),
                socket.emit("userData", e),
                socket.on("kicked", function () {
                    jQuery("#modalKicked").modal()
                }),
                ...
                socket.on("lobbyKicked", function (t) {
                    var e = game.players.get(t);
                    null != e && (game.chatAddMsg(null, e.name + " was kicked!", j),
                        e.kicked = !0)
                }),
                socket.on("chat", function (t) {
                    var e = game.players.get(t.id),
                        n = game.players.get(game.myID),
                        o = e.id == game.drawingID || e.guessedWord;
                    n.id != game.drawingID && !n.guessedWord && o || game.chatAddMsg(e, b(t.message), o ? "#7dad3f" : "#000")
                }),
                socket.on("lobbySpam", function () {
                    game.chatAddMsg(null, "Spam detected! You're sending too many messages.", j)
                }),
                socket.on("restartNotification", function (t) {
                    game && game.chatAddMsg(null, "Server will restart in about " + t + " seconds...", N)
                }),
                socket.on("lobbyVotekickCurrent", function (t) {
                    if (game) {
                        var e = game.players.get(t[0]),
                            n = game.players.get(t[1]);
                        game.chatAddMsg(null, "'" + e.name + "' is voting to kick '" + n.name + "' (" + t[2] + "/" + t[3] + ")", N)
                    }
                }),
                socket.on("lobbyConnected", function (t) {
                    if (ct.clear(),
                        E(""),
                        game = new GameClass(t),
                        game.inGame) {
                        for (var e = 0; e < t.drawCommands.length; e++)
                            B(t.drawCommands[e]);
                        at.goto("game"),
                            game.drawingID >= 0 && A(game.time)
                    } else
                        at.goto("lobby")
                }),
                socket.on("lobbyLobby", function () {
                    at.goto("lobby")
                }),
                ...
                socket.on("lobbyDisconnected", function () {
                    socket.emit("lobbyLeave"),
                        game = null,
                        at.goto("login")
                }),
                socket.on("lobbyGameStart", function (t) {
                    game.reset(),
                        E(""),
                        at.goto("game")
                }),
                socket.on("lobbyGameEnd", function () {
                    var t = [];
                    game.players.forEach(function (e, n, o) {
                            t.push(e)
                        }),
                        I({
                            mode: "endgame",
                ...
                socket.on("lobbyLanguage", function (t) {
                    game.setLanguage(t)
                }),
                socket.on("lobbyRounds", function (t) {
                    game.setRounds(t)
                }),
                socket.on("lobbyDrawTime", function (t) {
                    game.setDrawTime(t)
                }),
                socket.on("lobbyCustomWordsExclusive", function (t) {
                    game.setCustomWordsExclusive(t)
                }),
                socket.on("lobbyPlayerConnected", function (t) {
                    game.addPlayer(t),
                        game.chatAddMsg(null, t.name + " joined.", O),
                        it.playSound("playerJoin")
                }),
                socket.on("lobbyPlayerDisconnected", function (t) {
                    var e = game.players.get(t);
                    game.removePlayer(e),
                        e.kicked || game.chatAddMsg(null, e.name + " left.", j),
                        it.playSound("playerLeave")
                }),
                socket.on("lobbyPlayerGuessedWord", function (t) {
                    var e = game.players.get(t);
                    game.setPlayerGuessed(e),
                        game.chatAddMsg(null, e.name + " guessed the word!", U),
                        t == game.myID && game.chatDisable(),
                        it.playSound("playerGuessed")
                }),
                socket.on("lobbyGuessClose", function (t) {
                    game.chatAddMsg(null, "'" + t + "' is close!", N)
                }),
                socket.on("lobbyPlayerDrawing", function (t) {
                    var e = game.players.get(t);
                    game.setPlayerDrawing(e.id),
                        game.chatAddMsg(null, e.name + " is drawing now!", _);
                    var n = game.drawingID == game.myID;
                    ct.setDrawing(n),
                        n ? (f(),
                            y()) : (p(),
                            d()),
                        it.playSound("roundStart"),
                        A(game.timeMax),
                        R()
                }),
                socket.on("lobbyOwnerChanged", function (t) {
                    var e = game.players.get(t);
                    game.setPlayerOwner(e),
                        game.chatAddMsg(null, e.name + " is the owner now!", U)
                }),
                socket.on("lobbyRound", function (t) {
                    game.round = t,
                        game.updateRound(),
                        I({
                            mode: "text",
                ...
                socket.on("lobbyChooseWord", function (t) {
                    if (t.id == game.myID)
                        ...
                        I({
                            mode: "text",
                            text: game.players.get(t.id).name + " is choosing a word!"
                        })
                    }
                }),
                socket.on("lobbyReveal", function (t) {
                    C(),
                        game.chatEnable(),
                        game.chatAddMsg(null, "The word was '" + t.word + "'", U);
                    for (var e = [], n = !0, o = 0; o < t.scores.length / 2; o++) {
                        var r = t.scores[2 * o],
                            s = t.scores[2 * o + 1],
                            i = game.players.get(r);
                        if (i) {
                            i.guessedWord && (n = !1);
                            var a = s - i.score;
                            game.setPlayerScore(i, s, a),
                                e.push(i)
                        }
                    }
                    game.updateRanks(),
                        ...
                        game.setPlayerDrawing(-1),
                        ct.setDrawing(!1),
                        game.playersResetGuessedWord()
                }),
                socket.on("lobbyPlayerRateDrawing", function (t) {
                    var e = game.players.get(t[0]),
                        n = game.players.get(t[1]),
                        o = t[2];
                    e && game.playerRate(e, o),
                        n && (o > 0 ? n.thumbsUp++ : n.thumbsDown++)
                }),
                socket.on("lobbyCurrentWord", function (t) {
                    game.players.get(game.myID).guessedWord || E(t)
                }),
                socket.on("lobbyTime", function (t) {
                    D(t)
                })
        })
}
...
function D(t) {
    dt = t,
        dt <= 8 && it.playSound("tick"),
        jQuery("#timer").text(dt)
}
...
function E(t) {
    jQuery("#currentWord").text(t)
}

function M(t, e, n) {
    if (game.checkDrawing() && (ct.updateMousePosition(t, e, n),
            ct.brush.down)) {
        var o = null;
...
function T(t) {
    var e = t.clone();
    return e.removeAttr("id"),
        e.show(),
        e
}

function B(t) {
    game.drawingID == game.myID ? (ct.drawCommands.push(t),
        ct.performDrawCommand(t)) : ct.addDrawCommandReceived(t)
}
...
function L(t) {
    ...
    if ("choosewords" == t.mode) {
        ...
        for (var i = 0; i < t.words.length; i++) {
            var a = jQuery("<div class='word'></div>");
            a.data("id", i),
                a.text(t.words[i]),
                r.append(a),
                a.on("click", function () {
                    var t = jQuery(this).data("id");
                    socket && socket.emit("lobbyChooseWord", t),
                        R()
                })
        }
    } else
    ...
    if ("reveal" == t.mode) {
        c.text(t.revealReason),
            c.show(),
            u.empty(),
            u.show(),
            t.players.sort(function (t, e) {
                return e.scoreGuessed - t.scoreGuessed
            });
        for (var i = 0; i < t.players.length; i++) {
            var h = t.players[i],
                playerDiv = jQuery("<div class='player'></div>"),
                p = jQuery("<div class='name'></div>");
            p.text(h.name);
            var f = jQuery("<div class='score'></div>");
            h.scoreGuessed > 0 ? (f.text("+" + h.scoreGuessed),
                    f.css("color", "#07EA30")) : (f.text(h.scoreGuessed),
                    f.css("color", "#E81300")),
                playerDiv.append(p),
                playerDiv.append(f),
                u.append(playerDiv)
    ...
    if ("endgame" == t.mode) {
        ...
        var y = jQuery(".gameEndContainerPlayersBest"),
            m = jQuery(".gameEndContainerPlayers");
        y.empty(),
            m.empty();
        for (var i = 0; i < t.players.length; i++) {
            var h = t.players[i],
                playerDiv = T(jQuery("#gameEndPlayerDummy"));
            playerDiv.find(".name").text(h.name);
            var g = playerDiv.find(".rank");
            g.text("#" + h.rank),
                h.rank <= 3 && g.addClass("rank-" + h.rank),
                h.rank <= 3 ? y.append(playerDiv) : m.append(playerDiv),
                initFunc(playerDiv, h.avatar, h.id == game.ownerID, h.rank <= 3 ? 2 : 1)
        }
    } else
        d.hide();
    o.show(),
...
function R() {
    ...
    return function (t) {
        ...
        function o(t, e) {
            "object" === (void 0 === t ? "undefined" : s(t)) && (e = t,
                t = void 0),
            e = e || {};
            var n, o = i(t),
                a = o.source,
                ...
                p = o.path,
                f = h[l] && p in h[l].nsps;
            return e.forceNew || e["force new connection"] || !1 === e.multiplex || f ? (u("ignoring socket cache for %s", a),
                    n = c(a, e)) : (h[l] || (u("new io instance for %s", a),
                        h[l] = c(a, e)),
                    n = h[l]),
                o.query && !e.query ? e.query = o.query : e && "object" === s(e.query) && (e.query = r(e.query)),
                n.socket(o.path, e)
        }
        ...
        (function (e) {
            ...
            function o(t, n) {
                ...
                var i = -1 !== o.host.indexOf(":"),
                    a = i ? "[" + o.host + "]" : o.host;
                return o.id = o.protocol + "://" + a + ":" + o.port,
                    o.href = o.protocol + "://" + a + (n && n.port === o.port ? "" : ":" + o.port),
                    o
            }
            var r = n(2),
        ...
        t.exports = function (t) {
            ...
            for (var i = n.exec(t || ""), a = {}, c = 14; c--;)
                a[o[c]] = i[c] || "";
            return -1 != r && -1 != s && (a.source = e,
        ...
        (function (s) {
            function i() {
                return void 0 !== document && "WebkitAppearance" in document.documentElement.style || window.console && (console.firebug || console.exception && console.table) || navigator.userAgent.toLowerCase().match(/firefox\/(\d+)/) && parseInt(RegExp.$1, 10) >= 31
            }

            function a() {
                var t = arguments,
                    e = this.useColors;
                if (t[0] = (e ? "%c" : "") + this.namespace + (e ? " %c" : " ") + t[0] + (e ? "%c " : " ") + "+" + o.humanize(this.diff),
                    !e)
                    return t;
        ...
        function r(t) {
            ...
            function r() {
                ...
                for (var i = new Array(arguments.length), a = 0; a < i.length; a++)
                    i[a] = arguments[a];
                i[0] = e.coerce(i[0]),
                    "string" != typeof i[0] && (i = ["%o"].concat(i));
                var c = 0;
        ...
        function s(t) {
            e.save(t);
            for (var n = (t || "").split(/[\s,]+/), o = n.length, r = 0; r < o; r++)
                n[r] && (t = n[r].replace(/[\\^$+?.()|[\]{}]/g, "\\$&").replace(/\*/g, ".*?"),
        ...
        function a(t) {
            var n, o;
            for (n = 0,
                o = e.skips.length; n < o; n++)
                if (e.skips[n].test(t))
                    return !1;
            for (n = 0,
                o = e.names.length; n < o; n++)
                if (e.names[n].test(t))
                    return !0;
            return !1
        }
        ...
        function a(t) {
            ...
            return t.charAt(++o) && (n = c(n, t.substr(o))),
        ...
        (function (e, o) {
            ...
            (function () {
                function s(t, e) {
                    function n(t) {
                        ...
                        if ("bug-string-char-index" == t)
                            ...
                            s = n("json-stringify") && n("json-parse");
                        else {
                    ...
                    var o = t.Number || u.Number,
                        ...
                        p = t.Math || u.Math,
                        f = t.JSON || u.JSON;
                    "object" == typeof f && f && (e.stringify = f.stringify,
                        e.parse = f.parse);
                    ...
                    if (!n("json")) {
                        ...
                        if ((d = g.hasOwnProperty) || (d = function (t) {
                            ...
                            y = function (t, e) {
                                ...
                                for (r in o)
                                    d.call(o, r) && s++;
                                return n = o = null,
                                    s ? y = 2 == s ? function (t, e) {
                                        var n, o = {},
                                            r = v.call(t) == w;
                                        for (n in t)
                                            r && "prototype" == n || d.call(o, n) || !(o[n] = 1) || !d.call(t, n) || e(n)
                                    } :
                                    function (t, e) {
                                        var n, o, r = v.call(t) == w;
                                        for (n in t)
                                            r && "prototype" == n || !d.call(t, n) || (o = "constructor" === n) || e(n);
                            ...
                            var E = {
                                ...
                                B = function (t, e, n, o, r, s, i) {
                                    ...
                                    if ("object" == typeof a && a)
                                        if ("[object Date]" != (c = v.call(a)) || d.call(a, "toJSON"))
                                            "function" == typeof a.toJSON && (c != k && c != x && c != C || d.call(a, "toJSON")) && (a = a.toJSON(t));
                                        else if (a > -1 / 0 && a < 1 / 0) {
                                        if (P) {
                                            for (p = D(a / 864e5),
                                                u = D(p / 365.2425) + 1970 - 1; P(u + 1, 0) <= p; u++)
                                            ;
                            ...
                            e.stringify = function (t, e, n) {
                                ...
                                if (n)
                                    if ((i = v.call(n)) == k) {
                                        ...
                                        i == x && (o = n.length <= 10 ? n : n.slice(0, 10));
                                return B("", (c = {},
                                    c[""] = t,
                                    c), r, s, o, "", [])
                            }
                        ...
                        if (!n("json-parse")) {
                            var I, L, R = r.fromCharCode,
                                ...
                                _ = function () {
                                    for (var t, e, n, o, r, s = L, i = s.length; I < i;)
                                        switch (r = s.charCodeAt(I)) {
                                            case 9:
                                            case 10:
                                            case 13:
                                            case 32:
                                            ...
                                            case 58:
                                            case 44:
                                                return t = A ? s.charAt(I) : s[I],
                                                    I++,
                                                    t;
                                            case 34:
                                                for (t = "@",
                                                    I++; I < i;)
                                                    if ((r = s.charCodeAt(I)) < 32)
                                                        N();
                                                    else if (92 == r)
                                                    switch (r = s.charCodeAt(++I)) {
                                                        case 92:
                                                        ...
                                                        case 117:
                                                            for (e = ++I,
                                                                n = I + 4; I < n; I++)
                                                                (r = s.charCodeAt(I)) >= 48 && r <= 57 || r >= 97 && r <= 102 || r >= 65 && r <= 70 || N();
                                                            t += R("0x" + s.slice(e, I));
                                                            break;
                                                        default:
                                                            N()
                                            ...
                                            default:
                                                if (e = I,
                                                    45 == r && (o = !0,
                                                        r = s.charCodeAt(++I)),
                                                    r >= 48 && r <= 57) {
                                                    for (48 == r && (r = s.charCodeAt(I + 1)) >= 48 && r <= 57 && N(),
                                                        o = !1; I < i && (r = s.charCodeAt(I)) >= 48 && r <= 57; I++)
                                                    ;
                                                    ...
                                                    if (101 == (r = s.charCodeAt(I)) || 69 == r) {
                                                        for (r = s.charCodeAt(++I),
                                                            43 != r && 45 != r || I++,
                                                            n = I; n < i && (r = s.charCodeAt(n)) >= 48 && r <= 57; n++)
                                                        ;
                                                        n == I && N(),
                                                            I = n
                                                    }
                                    ...
                                    if ("$" == t && N(),
                                        ...
                                        if ("{" == t) {
                                            for (e = {};
                                                "}" != (t = _()); n || (n = !0))
                                                n && ("," == t ? "}" == (t = _()) && N() : N()),
                                                "," != t && "string" == typeof t && "@" == (A ? t.charAt(0) : t[0]) && ":" == _() || N(),
                                                e[t.slice(1)] = O(_());
                                            return e
                                        }
                                        N()
                ...
                if (!h || h.global !== h && h.window !== h && h.self !== h || (u = h),
                    c && !i)
                    s(u, c);
                else {
                    var l = u.JSON,
        ...
        function o(t, e) {
            return this instanceof o ? (t && "object" === (void 0 === t ? "undefined" : r(t)) && (e = t,
                    t = void 0),
                e = e || {},
                ...
                void(this.autoConnect && this.open())) : new o(t, e)
        }
                                ...
                                if (!l)
                                    if ("pong" === e.type && "probe" === e.data) {
                                        if (a('probe transport "%s" pong', t),
                                            ...
                                            !h)
                                            return;
                                        o.priorWebsocketSuccess = "websocket" === h.name,
                                            a('pausing current transport "%s"', p.transport.name),
                                            p.transport.pause(function () {
                                                l || "closed" !== p.readyState && (a("changing transport and sending upgrade packet"),
                                                    u(),
                                                    p.setTransport(h),
                                                    h.send([{
                                                        type: "upgrade"
        ...
        (function (e) {
            ...
            t.exports = function (t) {
                ...
                try {
                    if ("undefined" != typeof XDomainRequest && !r && s)
        ...
        (function (e) {
            ...
            function r(t) {
                if (c.call(this, t),
                    ...
                    var n = "https:" === location.protocol,
                        o = location.port;
                    o || (o = n ? 443 : 80),
                        this.xd = t.hostname !== e.location.hostname || o !== t.port,
                        this.xs = t.secure !== n
                } else
            ...
            t.exports = r,
                ...
                s.prototype.create = function () {
                        ...
                        try {
                            if (this.extraHeaders) {
                                ...
                                for (var r in this.extraHeaders)
                                    this.extraHeaders.hasOwnProperty(r) && n.setRequestHeader(r, this.extraHeaders[r])
                            }
                        } catch (t) {}
                    ...
                    e.document && (this.index = s.requestsCount++,
                ...
                s.prototype.cleanup = function (t) {
                    if (void 0 !== this.xhr && null !== this.xhr) {
                        ...
                        e.document && delete s.requests[this.index],
                            this.xhr = null
                    }
                },
                    ...
                    try {
                        ...
                        if ("application/octet-stream" === e)
                            ...
                            try {
                                ...
                                for (var n = new Uint8Array(this.xhr.response), o = [], r = 0, s = n.length; r < s; r++)
                                    o.push(n[r]);
                ...
                s.requestsCount = 0,
                s.requests = {},
                e.document && (e.attachEvent ? e.attachEvent("onunload", i) : e.addEventListener && e.addEventListener("beforeunload", i, !1))
        }).call(e, function () {
            return this
        }())
    }, function (t, e, n) {
        ...
        (function (t) {
            ...
            function r(t, n, o) {
                if (!n)
                    return e.encodeBase64Packet(t, o);
                var r = t.data,
                    s = new Uint8Array(r),
            ...
            function c(t, e, n) {
                for (var o = new Array(t.length), r = f(t.length, n), s = 0; s < t.length; s++)
                    ! function (t, n, r) {
                        e(n, function (e, n) {
                            o[t] = n,
                                r(e, o)
            ...
            e.encodePacket = function (e, n, s, a) {
                    ...
                    if (c && c.base64)
                        return o(e, a);
                    var u = v[e.type];
                    return void 0 !== e.data && (u += s ? d.encode(String(e.data)) : String(e.data)),
                        a("" + u)
                },
                ...
                e.decodePayload = function (t, n, o) {
                    if ("string" != typeof t)
                        return e.decodePayloadAsBinary(t, n, o);
                    "function" == typeof n && (o = n,
                        n = null);
                    var r;
                    if ("" == t)
                    ...
                    return t.length ? void c(t, o, function (t, e) {
                        ...
                        return e.forEach(function (t) {
                                var e = "string" == typeof t,
                                    n = t;
                                if (e) {
                                    for (var o = new Uint8Array(t.length), i = 0; i < t.length; i++)
                                        o[i] = t.charCodeAt(i);
                                    n = o.buffer
                                }
                                r[s++] = e ? 0 : 1;
                                for (var a = n.byteLength.toString(), i = 0; i < a.length; i++)
                                    r[s++] = parseInt(a[i]);
                                r[s++] = 255;
                ...
                e.encodePayloadAsBlob = function (t, n) {
                    function o(t, n) {
                        e.encodePacket(t, !0, !0, function (t) {
                            ...
                            for (var s = t instanceof ArrayBuffer ? t.byteLength : t.size, i = s.toString(), a = new Uint8Array(i.length + 1), r = 0; r < i.length; r++)
                                a[r] = parseInt(i[r]);
                            if (a[i.length] = 255,
                                k) {
            ...
            function o(t) {
                function n(t) {
                    ...
                    if (r(t)) {
                        ...
                        for (var s in t)
                            if (Object.prototype.hasOwnProperty.call(t, s) && n(t[s]))
                                return !0
                    }
                    return !1
                }
        ...
        t.exports = function (t, e, n) {
            ...
            for (var r = new Uint8Array(t), s = new Uint8Array(n - e), i = e, a = 0; i < n; i++,
                a++)
                s[a] = r[i];
            return s.buffer
        }
        ...
        (function (t, r) {
            ! function (s) {
                function i(t) {
                    for (var e, n, o = [], r = 0, s = t.length; r < s;)
                        e = t.charCodeAt(r++),
                        e >= 55296 && e <= 56319 && r < s ? (n = t.charCodeAt(r++),
                ...
                function u(t) {
                    if (0 == (4294967168 & t))
                        return v(t);
                    var e = "";
                    return 0 == (4294965248 & t) ? e = v(t >> 6 & 31 | 192) : 0 == (4294901760 & t) ? (e = v(t >> 12 & 15 | 224),
                            e += c(t, 6)) : 0 == (4292870144 & t) && (e = v(t >> 18 & 7 | 240),
                            e += c(t, 12),
                            e += c(t, 6)),
                        e += v(63 & t | 128)
                }
                ...
                function p() {
                    ...
                    if (240 == (248 & t) && (e = l(),
                            n = l(),
                            o = l(),
                            (r = (15 & t) << 18 | e << 12 | n << 6 | o) >= 65536 && r <= 1114111))
                        return r;
                ...
                var d = ("object" == typeof t && t && t.exports,
                    "object" == typeof r && r);
        ...
        (function (n) {
            ...
            a(s, i),
                ...
                s.prototype.doPoll = function () {
                    ...
                    o ? o.parentNode.insertBefore(n, o) : (document.head || document.body).appendChild(n),
                        this.script = n,
        ...
        (function (n) {
            function r(t) {
                t && t.forceBase64 && (this.supportsBinary = !1),
                    ...
                    this.usingBrowserWebSocket || (f = s),
                    i.call(this, t)
            }
            var s, i = o(26),
                a = o(27),
            ...
            f || void 0 !== window || (f = s),
                ...
                r.prototype.write = function (t) {
                    ...
                    var o = this;
                    this.writable = !1;
                    for (var r = t.length, s = 0, i = r; s < i; s++)
                        ! function (t) {
        ...
        (function (e) {
            var n = /^[\],:{}\s]*$/,
                ...
                a = /\s+$/;
            t.exports = function (t) {
                return "string" == typeof t && t ? (t = t.replace(i, "").replace(a, ""),
                    e.JSON && JSON.parse ? JSON.parse(t) : n.test(t.replace(o, "@").replace(r, "]").replace(s, "")) ? new Function("return " + t)() : void 0) : null
            }
        }).call(e, function () {
            return this
        }())
...
var $ = function (t, e) {
    ...
    K = function () {
        this.context = null,
            this.sounds = new Map,
            this.mute = !1,
            "true" === localStorage.getItem("audioMute") && this.setMute(!0),
            window.addEventListener("load", this.load.bind(this), !1)
    };
K.prototype.setMute = function (t) {
        this.mute = t,
            this.mute ? jQuery("#audio").css("background-image", "url(res/audio_off.gif") : jQuery("#audio").css("background-image", "url(res/audio.gif"),
            localStorage.setItem("audioMute", this.mute.toString())
    },
    K.prototype.addSound = function (t, e) {
        this.sounds.set(e, new $(this, t))
    },
    K.prototype.loadSounds = function () {
        this.addSound("res/sounds/roundStart.ogg", "roundStart"),
            this.addSound("res/sounds/roundEndSuccess.ogg", "roundEndSuccess"),
            this.addSound("res/sounds/roundEndFailure.ogg", "roundEndFailure"),
            this.addSound("res/sounds/join.ogg", "playerJoin"),
            this.addSound("res/sounds/leave.ogg", "playerLeave"),
            this.addSound("res/sounds/playerGuessed.ogg", "playerGuessed"),
            this.addSound("res/sounds/tick.ogg", "tick")
    },
...
var V = function (t) {
    this.canvas = t,
        ...
        this.canvasCtx.mozImageSmoothingEnabled = !1,
        this.canvasCtx.webkitImageSmoothingEnabled = !1,
        this.canvasCtx.msImageSmoothingEnabled = !1,
        this.canvasCtx.imageSmoothingEnabled = !1,
        this.mouseposPrev = {
            x: 0,
            y: 0
        },
...
V.prototype.updateMousePosition = function (t, e, n) {
        var o = this.canvas[0].getBoundingClientRect(),
            ...
            c = (t - o.left) / i,
            u = (e - o.top) / a;
        n ? (this.mouseposPrev.x = this.mousepos.x = Math.floor(c * r),
            this.mouseposPrev.y = this.mousepos.y = Math.floor(u * s)) : (this.mouseposPrev.x = this.mousepos.x,
            this.mouseposPrev.y = this.mousepos.y,
            this.mousepos.x = Math.floor(c * r),
            this.mousepos.y = Math.floor(u * s))
    },
    ...
    V.prototype.performDrawCommand = function (t) {
        switch (t[0]) {
            case 0:
                ...
                e < this.brush.thicknessMin && (e = this.brush.thicknessMin),
                    e > this.brush.thicknessMax && (e = this.brush.thicknessMax);
                var n = Math.floor(Math.ceil(e / 2)),
                    ...
                    u = hexToRGBObject(this.brush.getColor(t[1]));
            ...
            case 1:
                ...
                break;
            case 2:
                var u = hexToRGBObject(this.brush.getColor(t[1])),
    ...
    V.prototype.setPixel = function (t, e, n, o, r) {
        e >= 0 && e < t.data.length && (t.data[e] = n,
            t.data[e + 1] = o,
            t.data[e + 2] = r,
            t.data[e + 3] = 255)
    },
        ...
        if (n != a[0] || o != a[1] || r != a[2]) {
            for (var c = function (t) {
                ...
                for (d += 4 * h,
                    ...
                    y = !1,
                    m = !1; f++ < u - 1 && c(d);)
                    this.setPixel(s, d, n, o, r),
                    p > 0 && (c(d - 4) ? y || (i.push([p - 1, f]),
                        y = !0) : y && (y = !1)),
                    p < h - 1 && (c(d + 4) ? m || (i.push([p + 1, f]),
                        m = !0) : m && (m = !1)),
                    d += 4 * h
            }
                ...
                for (var n = -c; n <= c; n++)
                    for (var o = -c; o <= c; o++)
                        if (n * n + o * o < u) {
                            ...
                            r >= 0 && r < d.data.length && (d.data[r] = s,
                                d.data[r + 1] = i,
                                d.data[r + 2] = a,
                                d.data[r + 3] = 255)
                        }
    ...
    V.prototype.clear = function () {
        this.drawCommands = [],
            ...
            this.canvasCtx.fillRect(0, 0, this.canvas[0].width, this.canvas[0].height)
    },
    V.prototype.setDrawing = function (t) {
        t ? (this.brush.show(),
            jQuery(".containerToolbar").show()) : (this.brush.hide(),
            jQuery(".containerToolbar").hide())
    };
var Z = function () {
    this.down = !1,
        this.tool = "pen",
        ...
        this.setTool("pen"),
        this.setThickness(this.thicknessMin),
        this.setColor(1)
};
Z.prototype.hide = function () {
    ...
    Z.prototype.setDown = function (t) {
        this.down = t,
            this.down || (this.toolUsed = !1),
            jQuery("#cursor .tool").css("top", this.down ? 0 : -8)
    },
    Z.prototype.setTool = function (t) {
        this.tool = t,
            jQuery(".containerTools .tool").removeClass("toolActive"),
            jQuery(".containerTools .tool[data-tool='" + t + "']").addClass("toolActive"),
            this.updateBrushCursor()
    },
    Z.prototype.setColor = function (t) {
        this.colorIndex = t,
            jQuery(".colorPreview").css("background-color", this.getColor(t)),
            this.updateBrushCursor()
    },
    Z.prototype.getColor = function (t) {
        return jQuery(".colorItem[data-color='" + t + "']").css("background-color")
    },
    Z.prototype.setThickness = function (t) {
        this.thickness = t,
            this.thickness < this.thicknessMin && (this.thickness = this.thicknessMin),
            this.thickness > this.thicknessMax && (this.thickness = this.thicknessMax),
            this.updateBrushCursor()
    },
    Z.prototype.updateBrushCursor = function () {
        switch (this.tool) {
            ...
            case "erase":
                if (this.thickness <= 0)
                    return;
                this.brushCanvasCtx.clearRect(0, 0, this.thicknessMax, this.thicknessMax),
                    this.brushCanvasCtx.fillStyle = "erase" == this.tool ? "#FFF" : this.getColor(this.colorIndex),
                    this.brushCanvasCtx.beginPath(),
                    this.brushCanvasCtx.arc(this.thicknessMax / 2, this.thicknessMax / 2, this.thickness / 2 - 1, 0, 2 * Math.PI),
                    this.brushCanvasCtx.fill(),
                    this.brushCanvasCtx.strokeStyle = "#FFF",
                    this.brushCanvasCtx.beginPath(),
                    this.brushCanvasCtx.arc(this.thicknessMax / 2, this.thicknessMax / 2, this.thickness / 2 - 1, 0, 2 * Math.PI),
                    this.brushCanvasCtx.stroke(),
                    this.brushCanvasCtx.strokeStyle = "#000",
                    this.brushCanvasCtx.beginPath(),
                    this.brushCanvasCtx.arc(this.thicknessMax / 2, this.thicknessMax / 2, this.thickness / 2, 0, 2 * Math.PI),
                    this.brushCanvasCtx.stroke();
                var t = this.brushCanvas.toDataURL(),
                    e = this.thicknessMax / 2;
                jQuery("#canvasGame").css("cursor", "url(" + t + ")" + e + " " + e + ", default");
                break;
            case "fill":
                jQuery("#canvasGame").css("cursor", "url(res/fill_graphic.png) 7 38, default")
        }
    };
var GameClass = function (t) {
    this.name = t.name,
        this.key = t.key,
        this.language = t.language,
        this.slots = t.slots,
        this.drawingID = t.drawingID,
        this.myID = t.myID,
        this.ownerID = t.ownerID,
        this.round = t.round,
        this.roundMax = t.roundMax,
        this.time = t.time,
        this.timeMax = t.timeMax,
        this.useCustomWordsExclusive = t.useCustomWordsExclusive,
        this.inGame = t.inGame,
        this.players = new Map,
        this.containerGame = jQuery("#containerGamePlayers"),
        this.containerLobby = jQuery("#containerLobbyPlayers"),
        this.chatClear(),
        this.containerGamePlayerClear(),
        this.containerLobbyPlayerClear();
    for (var e = 0; e < t.players.length; e++)
        this.addPlayer(t.players[e]);
    jQuery("#invite").val("https://skribbl.io/?" + this.key),
        jQuery("#screenLobby .lobbySettings .lobbyName").text(this.name),
        this.containerLobbyUpdateRounds(),
        this.containerLobbyUpdateDrawTime(),
        this.containerLobbyUpdateUseCustomWordsExclusive(),
        this.containerLobbyUpdateLanguage(),
        this.containerLobbyPlayersUpdateOwner(),
        this.containerGamePlayersUpdateDrawing(),
        this.containerGamePlayersUpdateGuessed(),
        this.containerGamePlayersUpdateOwner(),
        this.updateRanks(),
        this.updateRound(),
        this.chatEnable()
};
GameClass.prototype.updatePlayerAvatars = function () {
        var t = this;
        this.players.forEach(function (e, n, o) {
            initFunc(t.containerGamePlayerGet(player), player.avatar, player.id == t.ownerID)
        })
    },
    GameClass.prototype.reset = function () {
        ...
        this.players.forEach(function (e, n, o) {
                e.guessedWord = !1,
                    ...
                    e.thumbsUp = 0,
                    e.thumbsDown = 0,
                    t.containerGamePlayerUpdateScore(e)
            }),
            this.containerGamePlayersUpdateDrawing(),
            this.containerGamePlayersUpdateGuessed(),
            this.containerGamePlayersUpdateOwner(),
    ...
    GameClass.prototype.checkDrawing = function () {
        return this.drawingID == this.myID
    },
    GameClass.prototype.addPlayer = function (t) {
        this.players.set(t.id, t),
            this.containerGamePlayerAdd(t),
            this.containerLobbyPlayerAdd(t),
            this.updateRanks()
    },
    GameClass.prototype.removePlayer = function (t) {
        this.containerGamePlayerRemove(t),
            this.containerLobbyPlayerRemove(t),
            this.players.delete(t.id),
            this.updateRanks()
    },
    GameClass.prototype.playersResetGuessedWord = function () {
        this.players.forEach(function (t, e, n) {
                t.guessedWord = !1
            }),
            this.containerGamePlayersUpdateGuessed()
    },
    GameClass.prototype.setPlayerOwner = function (t) {
        this.ownerID = t.id,
            this.containerGamePlayersUpdateOwner(),
            this.containerLobbyPlayersUpdateOwner()
    },
    GameClass.prototype.setPlayerDrawing = function (t) {
        this.drawingID = t,
            this.containerGamePlayersUpdateDrawing()
    },
    GameClass.prototype.setPlayerGuessed = function (t) {
        t.guessedWord = !0,
            this.containerGamePlayersUpdateGuessed()
    },
    GameClass.prototype.setPlayerScore = function (t, e, n) {
        t.score = e,
            t.scoreGuessed = n,
            this.containerGamePlayerUpdateScore(t)
    },
    GameClass.prototype.setLanguage = function (language) {
        this.language = language,
            this.containerLobbyUpdateLanguage()
    },
    GameClass.prototype.setRounds = function (t) {
        this.roundMax = t,
            this.updateRound(),
            this.containerLobbyUpdateRounds()
    },
    GameClass.prototype.setCustomWordsExclusive = function (t) {
        this.useCustomWordsExclusive = !!t,
            this.containerLobbyUpdateUseCustomWordsExclusive()
    },
    GameClass.prototype.setDrawTime = function (maxTime) {
        this.timeMax = maxTime,
            this.containerLobbyUpdateDrawTime()
    },
    GameClass.prototype.updateRound = function () {
        jQuery("#round").text("Round " + (this.round + 1) + " of " + this.roundMax)
    },
    GameClass.prototype.updateRanks = function () {
        var t = [];
        this.players.forEach(function (e, n, o) {
                t.push(e)
            }),
        ...
        for (var e = 1, n = 0; n < t.length; n++)
            t[n].rank = e,
            this.containerGamePlayerUpdateRank(t[n], e),
            n < t.length - 1 && t[n].score > t[n + 1].score && e++
    },
    GameClass.prototype.containerGamePlayerClear = function () {
        this.containerGame.empty()
    },
    GameClass.prototype.containerGamePlayerGet = function (t) {
        return this.containerGame.find("#player" + t.id)
    },
    GameClass.prototype.containerGamePlayerAdd = function (t) {
        var e = jQuery("#gamePlayerDummy").clone();
        e.attr("id", "player" + t.id),
            e.find(".name").text(t.name + (t.id == this.myID ? " (You)" : "")),
            t.id == this.myID && e.find(".name").css("color", "blue"),
            e.find(".drawing").hide(),
            this.containerGame.append(e),
            this.containerGamePlayerUpdateScore(t),
            e.show(),
            initFunc(e, t.avatar, t.id == this.ownerID, 1)
    },
    GameClass.prototype.containerGamePlayerRemove = function (t) {
        this.containerGamePlayerGet(t).remove()
    },
    GameClass.prototype.containerGamePlayersUpdateGuessed = function () {
        var t = this;
        this.players.forEach(function (e, n, o) {
    ...
    GameClass.prototype.containerGamePlayersUpdateDrawing = function () {
        var t = this;
        this.players.forEach(function (e, n, o) {
    ...
    GameClass.prototype.containerGamePlayersUpdateOwner = function () {
        var t = this;
        this.players.forEach(function (e, n, o) {
    ...
    GameClass.prototype.containerGamePlayerUpdateScore = function (t) {
        this.containerGamePlayerGet(t).find(".score").text("Points: " + t.score)
    },
    GameClass.prototype.containerGamePlayerUpdateRank = function (t, e) {
        this.containerGamePlayerGet(t).find(".rank").text("#" + e)
    },
    GameClass.prototype.containerLobbyPlayerGet = function (t) {
        return this.containerLobby.find("#player" + t.id)
    },
    GameClass.prototype.containerLobbyPlayerAdd = function (t) {
        var e = jQuery("#lobbyPlayerDummy").clone();
        e.attr("id", "player" + t.id),
            e.find(".name").text(t.name),
            e.find(".owner").hide(),
            t.id == this.myID && e.find(".you").show(),
            e.show(),
            this.containerLobby.append(e),
            initFunc(e, t.avatar, t.id == this.ownerID, 2)
    },
    GameClass.prototype.containerLobbyPlayerRemove = function (t) {
        this.containerLobbyPlayerGet(t).remove()
    },
    GameClass.prototype.containerLobbyPlayersUpdateOwner = function () {
        var t = this;
        this.players.forEach(function (e, n, o) {
        ...
        jQuery("#lobbySetRounds").prop("disabled", e),
            jQuery("#lobbySetDrawTime").prop("disabled", e),
            jQuery("#lobbySetCustomWords").prop("disabled", e),
            jQuery("#lobbyCustomWordsExclusive").prop("disabled", e),
            jQuery("#lobbySetLanguage").prop("disabled", e),
            jQuery("#buttonLobbyPlay").prop("disabled", e)
    },
    GameClass.prototype.containerLobbyPlayerClear = function () {
        this.containerLobby.empty()
    },
    GameClass.prototype.containerLobbyUpdateRounds = function () {
        jQuery("#lobbySetRounds").val(this.roundMax)
    },
    GameClass.prototype.containerLobbyUpdateDrawTime = function () {
        jQuery("#lobbySetDrawTime").val(this.timeMax)
    },
    GameClass.prototype.containerLobbyUpdateUseCustomWordsExclusive = function () {
        jQuery("#lobbyCustomWordsExclusive").attr("checked", !!this.useCustomWordsExclusive)
    },
    GameClass.prototype.containerLobbyUpdateLanguage = function () {
        var t = P(this.language);
        jQuery("#lobbySetLanguage").val(t)
    },
    GameClass.prototype.playerMessage = function (t, e) {
        this.playerMessageHandle(t);
        var n = this.containerGamePlayerGet(t).find(".message");
        n.find(".text").show(),
            n.find(".text").text(e)
    },
    GameClass.prototype.playerRate = function (t, e) {
        this.playerMessageHandle(t);
        var n = this.containerGamePlayerGet(t).find(".message");
        e > 0 ? n.find(".iconThumbsUp").show() : n.find(".iconThumbsDown").show()
    },
    GameClass.prototype.playerMessageHandle = function (t) {
        var e = this.containerGamePlayerGet(t).find(".message");
        e.find(".content").children().hide(),
            e.stop(),
            e.css("opacity", "1.0"),
            e.show(),
            t._previousHandle && clearTimeout(t._previousHandle),
            t._previousHandle = setTimeout(function () {
                e.animate({
                    opacity: "0"
                }, 500, function () {
                    e.hide()
    ...
    GameClass.prototype.chatClear = function () {
        jQuery("#boxMessages").empty()
    },
    GameClass.prototype.chatAddMsg = function (t, e, o) {
        var r = jQuery("#boxMessages"),
        ...
        if (a.css("color", o),
            ...
            jQuery("<b/>", {
                ...
                this.playerMessage(t, e)
        } else
            a.css("font-weight", "bold");
        jQuery("<span/>", {
                text: e
            }).appendTo(a),
            i && r.scrollTop(r[0].scrollHeight)
    },
    GameClass.prototype.chatDisable = function () {},
    GameClass.prototype.chatEnable = function () {};
var tt = function (t, e) {
    this.element = t,
        this.logo = e,
        this.onshow = null,
...
var ScreenManager = function () {
    this.screens = {
            load: new tt(jQuery("#screenLoading"), 0),
            login: new tt(jQuery("#screenLogin"), 2),
            lobby: new tt(jQuery("#screenLobby"), 1),
            game: new tt(jQuery("#screenGame"), 1)
        },
        this.screenCurrent = null
};
ScreenManager.prototype.goto = function (t) {
        this.screenCurrent && this.screens[this.screenCurrent].hide(),
            this.screenCurrent = t,
            this.screens[this.screenCurrent].show(),
            this.screens[this.screenCurrent].logo > 0 ? this.showLogo(this.screens[this.screenCurrent].logo) : this.hideLogo()
    },
    ScreenManager.prototype.hideLogo = function () {
        jQuery("#containerLogoBig").hide(),
            jQuery("#containerLogoSmall").hide()
    },
    ScreenManager.prototype.showLogo = function (t) {
        switch (this.hideLogo(),
            t) {
            case 1:
                jQuery("#containerLogoSmall").show();
                break;
            case 2:
                jQuery("#containerLogoBig").show()
        }
    };
var nt = window.location != window.parent.location ? document.referrer : document.location.href,
    ot = function (t, e) {
        return -1 !== t.indexOf(e)
    };
(ot(nt, "skribbl-io.com") || ot(nt, "skribblio.com") || ot(nt, "skribbl-io.org")) && (window.top.location.href = "https://www.skribbl.io"),
jQuery(function () {
...
var game = null,
    socket = null,
    it = new K,
    at = new ScreenManager,
    ct = new V(jQuery("#canvasGame")),
    ut = new Array,
    ht = !1,
    lt = !1;
ct.setDrawing(!1),
...
at.screens.game.onshow = function () {
    ...
    function () {
        for (var t = jQuery("#logoAvatarDummy"), e = jQuery("#logoAvatarContainer"), o = Math.round(8 * Math.random()), r = 0; r < 8; r++) {
            var i = T(t);
            e.append(i);
            var a = [0, 0, 0, 0];
            a[0] = r,
                a[1] = Math.round(100 * Math.random()) % H,
                a[2] = Math.round(100 * Math.random()) % F,
                a[3] = 1e3 * Math.random() < 70 ? Math.round(1 * Math.random()) : -1,
                initFunc(i, a, r == o, 1)
        }
    }(),
    jQuery("#inputName").change(function () {
        localStorage.setItem("name", jQuery("#inputName").val())
    }),
    null != localStorage.getItem("name") && jQuery("#inputName").val(localStorage.getItem("name")),
    jQuery("#loginLanguage").change(function () {
        localStorage.setItem("lang", jQuery("#loginLanguage").val())
    }),
    null != localStorage.getItem("lang") && jQuery("#loginLanguage").val(localStorage.getItem("lang")),
    loginAvatarData = function () {
        var t = localStorage.getItem("avatar");
        return t ? t = JSON.parse(t) : (t = [Math.round(Math.random() * z[0]), Math.round(Math.random() * z[1]), Math.round(Math.random() * z[2]), -1],
                localStorage.setItem("avatar", JSON.stringify(t))),
            t
    }(),
    jQuery("#buttonAvatarCustomizerRandomize").on("click", function () {
        return loginAvatarData[0] = Math.floor(Math.random() * z[0]),
            loginAvatarData[1] = Math.floor(Math.random() * z[1]),
            loginAvatarData[2] = Math.floor(Math.random() * z[2]),
            saveAvatarData(),
            initFunc(jQuery("#loginAvatar"), loginAvatarData, !1, 2),
            !1
    }),
    initFunc(jQuery("#loginAvatar"), loginAvatarData, !1, 2),
    jQuery("#loginAvatarArrowsLeft .avatarArrowLeft").on("click", function () {
        var t = parseInt(jQuery(this).data("avatarindex"));
        u(t, loginAvatarData[t] - 1)
    }),
    jQuery("#loginAvatarArrowsLeft .avatarArrowRight").on("click", function () {
        var t = parseInt(jQuery(this).data("avatarindex"));
        u(t, loginAvatarData[t] + 1)
    }),
    jQuery(document).ready(function () {
        ...
        function o() {
            if (game) {
                var e = game.checkDrawing() ? 90 : 190;
                r++,
                r == e && h(),
                    r >= e + 7 && window.location.reload(!0)
            }
        }
        var r = 0;
        setInterval(o, 1e3);
        jQuery(this).mousemove(e),
            jQuery(this).keypress(e),
            jQuery(this).on("mousedown", e),
            jQuery(this).on("touchmove", e)
    }),
    setInterval(l, 50),
    jQuery("#rateDrawing .thumbsUp").on("click", function () {
        socket && socket.emit("rateDrawing", 1),
            f()
    }),
    jQuery("#rateDrawing .thumbsDown").on("click", function () {
        socket && socket.emit("rateDrawing", 0),
            f()
    }),
    f(),
    y(),
    jQuery("#votekickCurrentplayer").on("click", m),
    jQuery("#formLogin").submit(function (t) {
        t.preventDefault(),
            lt = !1,
            g()
    }),
    jQuery("#buttonLoginCreatePrivate").on("click", function () {
        lt = !0,
            g()
    }),
    function (t, n) {
        ...
        r.async = "async",
            ...
            r.onload = r.onreadystatechange = function () {
                !s || r.readyState && !/loaded|complete/.test(r.readyState) || (s = !1,
                    n(),
...
var ft = null,
    dt = 0;
D(80),
    jQuery("#lobbySetLanguage").on("change", function () {
        socket && game.myID == game.ownerID && socket.emit("lobbySetLanguage", jQuery(this).val())
    }),
    jQuery("#lobbySetRounds").on("change", function () {
        socket && game.myID == game.ownerID && socket.emit("lobbySetRounds", jQuery(this).val())
    }),
    jQuery("#lobbySetDrawTime").on("change", function () {
        socket && game.myID == game.ownerID && socket.emit("lobbySetDrawTime", jQuery(this).val())
    }),
    jQuery("#lobbyCustomWordsExclusive:checkbox").change(function () {
        if (socket && game.myID == game.ownerID) {
            var t = this.checked;
            socket.emit("lobbySetCustomWordsExclusive", t)
        }
    }),
    jQuery("#inviteCopyButton").on("click", function () {
        document.querySelector("#invite").select();
        try {
            document.execCommand("copy")
        } catch (t) {}
    }),
    jQuery("#buttonJoinRandomLobby").on("click", function () {
        at.goto("load"),
            socket.emit("joinRandom")
    }),
    jQuery("#buttonLobbyLeave").on("click", S),
    jQuery("#buttonGameLeave").on("click", S),
    jQuery("#buttonLobbyPlay").on("click", function () {
        socket && game.myID == game.ownerID && socket.emit("lobbyGameStart", jQuery("#lobbySetCustomWords").val())
    }),
    jQuery("#formChat").submit(function (t) {
        var e = jQuery("#inputChat"),
            o = e.val();
        return o && (socket ? socket.emit("chat", o) : game.chatAddMsg(game.players.get(game.myID), o, "#000"),
                e.val("")),
            !1
    }),
    jQuery("#buttonOpenLobbyCreation").on("click", function () {
        jQuery("#modalCreateLobby").modal("show")
    }),
    jQuery("#buttonLobbyCreate").on("click", function () {
        var t = {
            name: jQuery("#lobbyCreateName").val(),
            private: jQuery("#lobbyCreatePrivate").prop("checked")
        };
        socket.emit("lobbyCreate", t),
            jQuery("#modalCreateLobby").modal("hide")
    }),
    jQuery("#buttonRefreshLobbies").on("click", function () {
    ...
    ct.canvas.on("mousedown", function (t) {
        switch (t.preventDefault(),
            t.button) {
            case 0:
                ct.brush.down || (ct.brush.setDown(!0),
                    M(t.clientX, t.clientY, !0))
        }
    });
var yt = null;
ct.canvas.on("touchstart", function (t) {
        ...
        e.length > 0 && null == yt && (yt = e[0].identitfier,
            ct.brush.setDown(!0),
            M(e[0].clientX, e[0].clientY, !0))
    }),
    ...
    jQuery(this).on("mouseup", function (t) {
        switch (t.button) {
            case 0:
                ct.brush.setDown(!1)
        }
    }),
    jQuery("#buttonClearCanvas").on("click", function () {
        socket ? socket.emit("canvasClear") : ct.clear()
    }),
    jQuery(".containerTools .tool").on("click", function () {
        ct.brush.setTool(jQuery(this).data("tool"))
    }),
    jQuery(".colorItem").on("click", function () {
        var t = jQuery(this).data("color");
        ct.brush.setColor(Number(t))
    }),
    jQuery(".brushSize").on("click", function () {
        var t = ct.brush.thicknessMin,
            e = ct.brush.thicknessMax,
            o = Number(jQuery(this).data("size")) * (e - t) + t;
